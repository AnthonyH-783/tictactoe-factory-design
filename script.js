...
const init = (function (){
    ...
    const bottom_row = gameboard.querySelector("#bottom-row");
    ...
    const scores_template = bottom_row.querySelector("#scores-template").innerHTML;

    // Initial rendering
    turn_wrapper.innerHTML = Mustache.render(turn_template, X);
    bottom_row.innerHTML = Mustache.render(scores_template, scores);
...
const gameBoard = (function () {
    ...
    function createGameBoard(){
        const list = new Array(GRID_SIZE).fill(new Array(GRID_SIZE).fill(" "));
        return list;
    }
    function reset(){
        if(!board){
    ...
    function start(){
        ...
        let move_num = 0;
        while(move_num < 9){
            let player = (move_num % 2 === 0) ? first_player : second_player;
